...
to go
  ...
  if time:get "day" dt = 16 and time:get "month" dt = 12 and time:get "year" dt = 2050 and time:get "hour" dt = 10 [export-visit-map export-home-range-sizes set hpc-stop "yes"]
...
to birth
  ...
  ask deer with [sex-of-deer = "female" and offspring = "no" and mature = "yes"] [ ; For female deer which do not have offspring
    ...
    if rand-number > (1 - annual-birth-prob) ^ (1 / 730.5) [ ; Need to do calculate from the prob of not giving birth because the only way to not give birth is to not give birth in every 12 hour step, so the maths is easy.
      set offspring "yes"
      hatch 1 [
...
to death
  ...
  ; Survival rates from 95-99% - how to model?

  ask deer [if age > max-lifespan-years * 730.5 [die]] ; Deer dies due to intrinsic senescnenc

  foreach sort deer [t -> ask t [
    let rand-number random-float 1
    if mature = "yes" and rand-number > (annual-survival-prob) ^ (1 / 730.5) [
...
@#$#@#$#@
<experiments>
  <experiment name="100 sims" repetitions="100" runMetricsEveryStep="true">
    <setup>clear-all
random-seed behaviorspace-run-number
setup</setup>
    <go>go</go>
    <exitCondition>hpc-stop = "yes"</exitCondition>
    <metric>count deer</metric>
    <metric>count deer with [mature = "yes"]</metric>
    <metric>count deer with [mature = "no"]</metric>
    ...
    <enumeratedValueSet variable="se-artificial:step-length:summerr:female">
      <value value="0"/>
    </enumeratedValueSet>
