...
; Movement - have tweaked here so that deer are always at the centre of the patch
...
to move-adult
  ...
  ask deer with [mother = "none" and mature = "yes"]  [ ; Asking the deer (this ask deer individually in a random order) who is mature and independent of a mother
    ...
    set target-patches patches in-radius max-step-distance with [land = "yes" and current-patch != self] ; creates an agentset of land patches within the max-step distance
...
to death
  ...
  ask deer [if age > max_lifespan_years * 730.5 [die]] ; Deer dies due to intrinsic senescnenc
...
to move-deer-based-on-probability; Moves deer to a new patch based on the calculated probabilities
  ...
  ifelse selected-patch != nobody [ ; If a patch is selected
    ...
    move-to selected-patch ; Jumps the deer to the middle of the selected-patch - I've done this because 'in-radius' uses the centre of the patch, so the probabilities are based on distance to the centre of patches.
  ] [
